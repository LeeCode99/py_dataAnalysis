...
class BookForm(FlaskForm):
    ...
    Genre = StringField('Genre', validators=[Length(min=0, max=30)])
...
def Welcome():
    ...
    if not Shelf.query.filter_by(Name='Reading').first():
        ...
        shelf4 = Shelf(Name='Recommendation')
        shelf5 = Shelf(Name='Favorite')
        db.session.add(shelf)
        db.session.add(shelf2)
        db.session.add(shelf3)
        db.session.add(shelf4)
        db.session.add(shelf5)
        db.session.commit()
...
def AddBook():
    ...
    if form.validate_on_submit():
        new_book = Book(Name=form.Name.data, Date=form.Date.data, Author=form.Author.data, Shelfname=form.Shelfname.data, Genre = form.Genre.data, FinishReading=form.FinishReading.data)
...
@app.route('/DisplayRecommendation/<name>', methods=['GET'])
def DisplayRecommendation(name):
    Books = Book.query.all()
    # convert Books shelfname to list
    genre = []
    for book in Books:
        genre.append(book.Genre)
    
    
    most_frequent_Genre = max(set(genre), key = genre.count)

    return render_template('DisplayRecommendation.html', Books=Books, name = name, most_frequent_Genre = most_frequent_Genre)
